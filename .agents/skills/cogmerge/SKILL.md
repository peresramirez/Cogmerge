...
python3 .agents/skills/cogmerge/scripts/smoke.py   # must print OK
```
...
Scripts live in `.agents/skills/cogmerge/scripts/`. They all `import backend`
...
4. **Write it:**
   ```bash
   python3 .agents/skills/cogmerge/scripts/seal.py record.json
   ```
...
1. **Retrieve:**
   ```bash
   python3 .agents/skills/cogmerge/scripts/check.py --base main --head HEAD
   ```
